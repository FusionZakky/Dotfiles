...
hl.bind("XF86AudioRaiseVolume", hl.dsp.exec_cmd("wpctl set-volume @DEFAULT_AUDIO_SINK@ 2%+ -l 1.5"), { locked = true })
hl.bind("XF86AudioLowerVolume", hl.dsp.exec_cmd("wpctl set-volume @DEFAULT_AUDIO_SINK@ 2%-"), { locked = true })
hl.bind("SUPER + W", hl.dsp.global("quickshell:wallpaperPickerToggle"), { description = "My custom wallpaper picker" })
...
-- end
--# keypad numbers
-- REVIEW: mirrors the numpad "send to workspace" loop above; not present in any recoverable backup, inferred from symmetry. Verify it does what you want.
for i = 1, 10 do
...
-- REVIEW: prefix values inferred from r-/r+ (relative workspace) and m-/m+ (relative monitor) conventions used elsewhere in this file. Verify behaviour.
do
...
-- REVIEW: no recoverable backup contains an actual bind statement for this block, and its two arrays
-- don't even match in length (2 vs 4), meaning it was already broken before this corruption. Left
-- disabled rather than guessed. Re-author below if you want scroll-wheel workspace focus-switching.
...
hl.bind("SUPER + ALT + F11", hl.dsp.exec_cmd([[notify-send "F11 test" "This is a simple test notification" -a "Hyprland"]]), { description = "Test: F11 notification" })

hl.bind("SUPER + ALT + F12", hl.dsp.exec_cmd([[notify-send "F12 test" "Another test notification" -a "Hyprland"]]), { description = "Test: F12 notification" })

hl.bind("SUPER + ALT + Equal", hl.dsp.exec_cmd([[notify-send "Urgent notification" "Ah hell no" -u critical -a "Hyprland keybind"]]), { description = "Test: urgent notification" })
