...
Singleton {
    ...
    property var providers: {
        ...
        "wallhaven": {
            ...
            "description": Translation.tr("Real photos & desktop wallpapers | High quality, curated"),
            "mapFunc": (response) => {
                ...
                return response.map(item => {
                    return {
                        ...
                        "md5": `${item.id}`,
                        "preview_url": item.thumbs?.large ?? item.thumbs?.small,
                        "sample_url": item.thumbs?.large ?? item.path,
                        "file_url": item.path,
                        "file_ext": item.path.split('.').pop(),
                        "source": (item.source && item.source.length > 0) ? item.source : item.url,
                    }
                })
            },
        },
    ...
    function constructRequestUrl(tags, nsfw=true, limit=20, page=1) {
        ...
        if (currentProvider === "zerochan") {
            ...
            const wallhavenApiKey = Config.options?.sidebar?.booru?.wallhaven?.apiKey
